...
class UpsampleBlock(nn.Module):
    ...
    def __init__(self, n_channel, n_classes=None, skip_in=0, use_bn=True, parametric=False):
        super(UpsampleBlock, self).__init__()

        self.parametric = parametric
        n_classes = n_channel/2 if n_classes is None else n_classes

        # first convolution: either transposed conv, or conv following the skip connection
        if parametric:
            # versions: kernel=4 padding=1, kernel=2 padding=0
            self.up = nn.ConvTranspose2d(in_channels=n_channel, out_channels=n_classes, kernel_size=(4, 4),
                                         stride=2, padding=1, output_padding=0, bias=(not use_bn))
            self.bn1 = nn.BatchNorm2d(n_classes) if use_bn else None
        else:
            self.up = None
            n_channel = n_channel + skip_in
            self.conv1 = nn.Conv2d(in_channels=n_channel, out_channels=n_classes, kernel_size=(3, 3),
        ...
        # second convolution
        conv2_in = n_classes if not parametric else n_classes + skip_in
        self.conv2 = nn.Conv2d(in_channels=conv2_in, out_channels=n_classes, kernel_size=(3, 3),
                               stride=1, padding=1, bias=(not use_bn))
        self.bn2 = nn.BatchNorm2d(n_classes) if use_bn else None

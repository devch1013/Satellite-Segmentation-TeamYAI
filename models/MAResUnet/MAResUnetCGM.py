...
class MAResUNetCGM(nn.Module):
    def __init__(self, num_channels=3, num_classes=5, filters=[64, 128, 256, 512]):
        super(MAResUNetCGM, self).__init__()
        self.name = "MAResUNet"
    ...
    def forward(self, x):
        ...
        return torch.sigmoid(out), cls
        # return out


if __name__ == "__main__":
    num_classes = 1
    in_batch, inchannel, in_h, in_w = 10, 3, 224, 224
    x = torch.randn(in_batch, inchannel, in_h, in_w)
    net = MAResUNetCGM(3, num_classes=num_classes, filters=[256, 512, 1024, 2048])
    out = net(x)
    print(out.shape)
